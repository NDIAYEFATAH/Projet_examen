    ...
    <x-guest-layout>
        <form method="POST" action="{{ route('saveguichet') }}" enctype="multipart/form-data">
            ...
            <div>
                ...
                <x-text-input id="formFile" class="block mt-1 w-full" type="text" name="numero_cni" :value="old('numero_cni')" required autofocus autocomplete="numero_cni" />
                <x-input-error :messages="$errors->get('numero_cni')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="name" :value="__('Votre Piece')" />
                <x-text-input id="cni_file" class="block mt-1 w-full" type="file" name="cni_file" :value="old('cni_file')" required autofocus autocomplete="cni_file" />
